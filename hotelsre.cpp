...
// Description : http://www.spoj.com/problems/HOTELS/
// Status	   : AC
//============================================================================
...
int main(){
	...
	prefix[0] = 0;
	int ans = a[0];
	repn(i,1,n+1) prefix[i] = prefix[i-1] + a[i-1];
	repn(i,1,n+1) trace1(prefix[i]);
	repn(i,1,n+1){
		trace1(prefix[i]);
		int t,req;
		t = prefix[upper_bound(prefix+1,prefix+1+n,prefix[i]+m)-(prefix+1)];
		trace1(t);
		if(t==prefix[n] && i==1){
			req = t;
		}
